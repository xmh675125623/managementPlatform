...
const menuArrange = (menu) => {
  menu.forEach((item) => {
    if (item.children.length < 1) {
      delete item.children
    } else {
      menuArrange(item.children)
    }
  })
}
...
export default {
  ...
  actions: {
    ...
    async login ({ commit, dispatch }, {
      ...
      const res = await SYS_USER_LOGIN({ username, password, validate_code })
      // 设置 cookie 一定要存 uuid 和 token 两个 cookie
      // 整个系统依赖这两个数据进行校验和存储
      // uuid 是用户身份唯一标识 用户注册的时候确定 并且不可改变 不可重复
      // token 代表用户当前登录状态 建议在网络请求中携带 token
      // 如有必要 token 需要定时更新，默认保存一天
      util.cookies.set('uuid', res.aid)
      ...
      // 设置 vuex 用户信息
      await dispatch('d2admin/user/set', { name: res.uname }, { root: true })
