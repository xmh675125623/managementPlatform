<template>
  <d2-container>
    <template slot="header">网关日志查看</template>
...
export default {
  computed: {
    ...mapState('plat/gatewayLog', ['tableName', 'tableList', 'logList', 'logCount', 'listLoading', 'currentPage',
      'pageSize', 'searchCondition', 'isOpenCondition', 'exportLoading']),
    level: {
      get () {
        return this.$store.state.plat.gatewayLog.searchCondition.level
      },
      set (value) {
        this.$store.commit('plat/gatewayLog/setLevel', value)
      }
    },
    context: {
      get () {
        return this.$store.state.plat.gatewayLog.searchCondition.context
      },
      set (value) {
        this.$store.commit('plat/gatewayLog/setContext', value)
      }
    },
    module: {
      get () {
        return this.$store.state.plat.gatewayLog.searchCondition.module
      },
      set (value) {
        this.$store.commit('plat/gatewayLog/setModule', value)
      }
    },
    sip: {
      get () {
        return this.$store.state.plat.gatewayLog.searchCondition.sip
      },
      set (value) {
        this.$store.commit('plat/gatewayLog/setSip', value)
      }
    },
    dip: {
      get () {
        return this.$store.state.plat.gatewayLog.searchCondition.dip
      },
      set (value) {
        this.$store.commit('plat/gatewayLog/setDip', value)
      }
    }
  },
  data () {
  ...
  methods: {
    ...mapActions('plat/gatewayLog', ['getLogList', 'exportLog']),
    ...
    handleResetForm () {
      this.$store.commit('plat/gatewayLog/setLevel', [])
      this.$store.commit('plat/gatewayLog/setContext', '')
      this.$store.commit('plat/gatewayLog/setModule', [])
      this.$store.commit('plat/gatewayLog/setSip', '')
      this.$store.commit('plat/gatewayLog/setDip', '')
    },
    rowStyle (data) {
      return levelcolors[data.row.level]
    },
    toggleForm () {
      this.$store.commit('plat/gatewayLog/toggleCondition')
    },
    handleSelectionChange (val) {
      this.selectedRows = val
    },
    handleSortChange (sort) {
      this.$store.commit('plat/gatewayLog/setSortField', { sortField: sort.prop, sortOrder: sort.order })
      this.getLogList({ table_name: this.tableName, page: 1, pageSize: this.pageSize })
    },
    printLog () {
