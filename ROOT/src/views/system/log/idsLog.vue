<template>
  <d2-container>
    <template slot="header">IDS日志查看</template>
...
export default {
  computed: {
    ...mapState('plat/idsLog', ['tableName', 'tableList', 'logList', 'logCount', 'listLoading', 'currentPage',
      'pageSize', 'searchCondition', 'isOpenCondition', 'exportLoading']),
    level: {
      get () {
        return this.$store.state.plat.idsLog.searchCondition.level
      },
      set (value) {
        this.$store.commit('plat/idsLog/setLevel', value)
      }
    },
    context: {
      get () {
        return this.$store.state.plat.idsLog.searchCondition.context
      },
      set (value) {
        this.$store.commit('plat/idsLog/setContext', value)
      }
    },
    module: {
      get () {
        return this.$store.state.plat.idsLog.searchCondition.module
      },
      set (value) {
        this.$store.commit('plat/idsLog/setModule', value)
      }
    },
    sip: {
      get () {
        return this.$store.state.plat.idsLog.searchCondition.sip
      },
      set (value) {
        this.$store.commit('plat/idsLog/setSip', value)
      }
    },
    dip: {
      get () {
        return this.$store.state.plat.idsLog.searchCondition.dip
      },
      set (value) {
        this.$store.commit('plat/idsLog/setDip', value)
      }
    }
  },
  data () {
  ...
  methods: {
    ...mapActions('plat/idsLog', ['getLogList', 'exportLog']),
    ...
    handleResetForm () {
      this.$store.commit('plat/idsLog/setLevel', [])
      this.$store.commit('plat/idsLog/setContext', '')
      this.$store.commit('plat/idsLog/setModule', [])
      this.$store.commit('plat/idsLog/setSip', '')
      this.$store.commit('plat/idsLog/setDip', '')
    },
    rowStyle (data) {
      return levelcolors[data.row.level]
    },
    toggleForm () {
      this.$store.commit('plat/idsLog/toggleCondition')
    },
    handleSelectionChange (val) {
      this.selectedRows = val
    },
    handleSortChange (sort) {
      this.$store.commit('plat/idsLog/setSortField', { sortField: sort.prop, sortOrder: sort.order })
      this.getLogList({ table_name: this.tableName, page: 1, pageSize: this.pageSize })
    },
    printLog () {
