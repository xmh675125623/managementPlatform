<template>
  <d2-container>
    <template slot="header">隔离日志查看</template>
...
<!--      <el-table-column type="selection" width="55" :reserve-selection="true"></el-table-column>-->
      ...
      <el-table-column label="事件等级" width="120" prop="severity" sortable="custom">
        <template v-slot="scope">
...
const modules = { 1: '自定义策略', 2: 'MODBUS_TCP', 6: '双机热备' }
const levelArray = [{ label: 'Emerg', value: 0 }, { label: 'Alert', value: 1 }, { label: 'Critical', value: 2 },
  ...
  { label: 'Debug', value: 7 }]
const moduleArray = [{ label: '自定义策略', value: 1 }, { label: 'MODBUS_TCP', value: 2 },]
const severitys = { 0: 'Emergency', 1: 'Alert', 2: 'Critical', 3: 'Error', 4: 'Warning', 5: 'Notice', 6: 'Informational', 7: 'Debug' }

export default {
  computed: {
    ...mapState('plat/isolationLog', ['tableName', 'tableList', 'logList', 'logCount', 'listLoading', 'currentPage',
      'pageSize', 'searchCondition', 'isOpenCondition', 'exportLoading']),
    level: {
      get () {
        return this.$store.state.plat.isolationLog.searchCondition.level
      },
      set (value) {
        this.$store.commit('plat/isolationLog/setLevel', value)
      }
    },
    context: {
      get () {
        return this.$store.state.plat.isolationLog.searchCondition.context
      },
      set (value) {
        this.$store.commit('plat/isolationLog/setContext', value)
      }
    },
    module: {
      get () {
        return this.$store.state.plat.isolationLog.searchCondition.module
      },
      set (value) {
        this.$store.commit('plat/isolationLog/setModule', value)
      }
    },
    sip: {
      get () {
        return this.$store.state.plat.isolationLog.searchCondition.sip
      },
      set (value) {
        this.$store.commit('plat/isolationLog/setSip', value)
      }
    },
    dip: {
      get () {
        return this.$store.state.plat.isolationLog.searchCondition.dip
      },
      set (value) {
        this.$store.commit('plat/isolationLog/setDip', value)
      }
    }
  },
  data () {
  ...
  methods: {
    ...mapActions('plat/isolationLog', ['getLogList', 'exportLog']),
    ...
    handleResetForm () {
      this.$store.commit('plat/isolationLog/setLevel', [])
      this.$store.commit('plat/isolationLog/setContext', '')
      this.$store.commit('plat/isolationLog/setModule', [])
      this.$store.commit('plat/isolationLog/setSip', '')
      this.$store.commit('plat/isolationLog/setDip', '')
    },
    rowStyle (data) {
      return levelcolors[data.row.level]
    },
    toggleForm () {
      this.$store.commit('plat/isolationLog/toggleCondition')
    },
    handleSelectionChange (val) {
      this.selectedRows = val
    },
    handleSortChange (sort) {
      this.$store.commit('plat/isolationLog/setSortField', { sortField: sort.prop, sortOrder: sort.order })
      this.getLogList({ table_name: this.tableName, page: 1, pageSize: this.pageSize })
    },
    printLog () {
