<template>
  <d2-container>
    <template slot="header">审计日志管理</template>
...
export default {
  computed: {
    ...mapState('plat/auditLogManage', ['tableList', 'listLoading', 'deleteLoading', 'exportLoading']),
    exportTableLoading () {
      return (row) => {
  ...
  methods: {
    ...mapActions('plat/auditLogManage', ['getTableList', 'deleteTable', 'exportTable']),
    handleClickSelectFile () {
      const data = {
        method: 'plat.disk_audit.import',
