<template>
  <d2-container>
    <template slot="header">防火墙日志管理</template>
...
export default {
  computed: {
    ...mapState('plat/firewallLogManage', ['tableList', 'listLoading', 'deleteLoading', 'exportLoading']),
    exportTableLoading () {
      return (row) => {
  ...
  methods: {
    ...mapActions('plat/firewallLogManage', ['getTableList', 'deleteTable', 'exportTable']),
    handleClickSelectFile () {
      const data = {
        method: 'plat.disk_firewall.import',
