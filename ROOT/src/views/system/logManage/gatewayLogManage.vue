...
export default {
  computed: {
    ...mapState('plat/gatewayLogManage', ['tableList', 'listLoading', 'deleteLoading', 'exportLoading']),
    exportTableLoading () {
      return (row) => {
  ...
  methods: {
    ...mapActions('plat/gatewayLogManage', ['getTableList', 'deleteTable', 'exportTable']),
    handleClickSelectFile () {
      const data = {
        method: 'plat.disk_gateway.import',
