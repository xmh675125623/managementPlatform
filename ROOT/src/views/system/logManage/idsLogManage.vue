<template>
  <d2-container>
    <template slot="header">IDS日志管理</template>
...
export default {
  computed: {
    ...mapState('plat/idsLogManage', ['tableList', 'listLoading', 'deleteLoading', 'exportLoading']),
    exportTableLoading () {
      return (row) => {
  ...
  methods: {
    ...mapActions('plat/idsLogManage', ['getTableList', 'deleteTable', 'exportTable']),
    handleClickSelectFile () {
      const data = {
        method: 'plat.disk_ids.import',
