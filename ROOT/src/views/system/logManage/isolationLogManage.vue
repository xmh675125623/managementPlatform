<template>
  <d2-container>
    <template slot="header">隔离日志管理</template>
...
export default {
  computed: {
    ...mapState('plat/isolationLogManage', ['tableList', 'listLoading', 'deleteLoading', 'exportLoading']),
    exportTableLoading () {
      return (row) => {
  ...
  methods: {
    ...mapActions('plat/isolationLogManage', ['getTableList', 'deleteTable', 'exportTable']),
    handleClickSelectFile () {
      const data = {
        method: 'plat.disk_isolation.import',
