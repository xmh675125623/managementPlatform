          ...
          <div class="page-login--form">
            <el-card shadow="never">
              <el-form ref="loginForm" label-position="top" :rules="rules" :model="formLogin" size="default">
                ...
                <el-form-item prop="code">
                  <el-input type="text" v-model="formLogin.code" placeholder="验证码" @keyup.enter.native="submit">
                    <template slot="append">
                      <img
                        class="login-code"
                        :src="`${validateCodeUrl}?token=${this.loginToken}&date=${this.tokenDate}`"
                ...
                <el-button size="default" @keyup.enter.native="submit" @click="submit" type="primary" class="button-login">
                  登录
                </el-button>
              </el-form>
            </el-card>
...
import { loginValidateCodeUrl } from '@/api/_service'
export default {
  mixins: [
    localeMixin
  ],
  ...
  mounted () {
    // 获取登录token用于验证码图片获取
    this.validateCodeUrl = loginValidateCodeUrl
    this.loginToken = localStorage.getItem('login_token')
    if (!this.loginToken || this.loginToken.length !== 37) {
      this.loginToken = this.guid()
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          this.login({
            ...
            validate_code: this.formLogin.code
          }).then(() => {
            // 重定向对象不存在则返回顶层路径
